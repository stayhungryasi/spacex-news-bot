...
QUERY = "Anthropic OR Claude AI OR Claude Opus OR Claude Sonnet"
TOP_N = 5
...
log = logging.getLogger("anthropic-news-bot")
...
# Primary relevance gate: the article must explicitly mention Anthropic. "Claude"
# alone is far too noisy — it's a common personal name (Claude Monet, Claude
# Debussy, ...) and articles about OpenAI / ChatGPT / Gemini frequently name-drop
# "Claude" without being about Anthropic at all.
PRIMARY_KEYWORD = "anthropic"
# Secondary supplement terms (used only to fill TOP_N when primary < 5). These
# are specific enough to strongly imply the Anthropic model/company, but we still
# require an AI/tech context marker below to avoid false positives.
SECONDARY_KEYWORDS = ("claude ai", "claude code", "claude opus", "claude sonnet", "dario amodei")
# AI/tech context markers — a secondary-tier article must contain at least one so
# a stray "Claude" (person's name) in a non-tech piece can't sneak through.
AI_CONTEXT_MARKERS = (
    "anthropic",
    "ai",
...
# Substantive-news context terms. A single passing mention of "Anthropic" only
# counts as primary when paired with one of these — filters out "I used Anthropic
# Claude to..." personal write-ups that mention the company just once in passing.
...
# Source (domain / source name) blacklist — personal blog and UGC platforms whose
# "I tried Claude" posts are not news. Matched case-insensitively as substrings
# against both the article URL and the NewsAPI source name.
SOURCE_BLACKLIST = (
    "blogger.com",
    "blogspot",
    "medium.com",
    ...
    "cryptobriefing",
    "crypto briefing",
    # Package registries — "added to PyPI / supports Anthropic API" listings are
    # registration records, not news.
    "pypi.org",
    "npmjs.com",
    "github.com",
    "libraries.io",
...
# Title-pattern blacklist — first-person experiment / experience posts and
# package-registration notices. Matched case-insensitively as substrings against
# the article title.
...
def _is_blacklisted_source(article: dict) -> bool:
    url = (article.get("url") or "").lower()
    source_name = ((article.get("source") or {}).get("name") or "").lower()
    return any(b in url or b in source_name for b in SOURCE_BLACKLIST)


def _has_blacklisted_title(article: dict) -> bool:
    title = (article.get("title") or "").lower()
    return any(p in title for p in TITLE_BLACKLIST_PATTERNS)


def _match_tier(article: dict) -> str | None:
    title = (article.get("title") or "").lower()
    text = _haystack(article)
    # Strengthened primary gate — a substantive Anthropic story must satisfy one
    # of: Anthropic in the title, Anthropic mentioned 2+ times in the body, or
    # Anthropic alongside a hard-news context term. A lone passing mention
    # ("used Anthropic Claude") no longer qualifies.
    if PRIMARY_KEYWORD in text:
        if (
            PRIMARY_KEYWORD in title
            or text.count(PRIMARY_KEYWORD) >= 2
            or any(term in text for term in PRIMARY_CONTEXT_TERMS)
        ):
            return "primary"
    if any(kw in text for kw in SECONDARY_KEYWORDS) and any(
        m in text for m in AI_CONTEXT_MARKERS
    ):
        return "secondary"
    return None


def filter_anthropic(articles: list[dict]) -> list[tuple[str, dict]]:
    """Return all Anthropic-related candidates, primary tier first.

    - primary: Anthropic in the title, OR mentioned 2+ times in the body, OR
      mentioned alongside a hard-news context term (funding / valuation / launch
      / release / lawsuit / partnership / Dario Amodei / safety / enterprise /
      model). A lone passing mention is not enough.
    - secondary: contains a specific Anthropic term (Claude AI / Claude Code /
      Claude Opus / Claude Sonnet / Dario Amodei) AND an AI/tech context marker
    Blacklisted sources (personal blog / UGC platforms) and first-person
    experiment/experience titles ("Ask AI:", "I tried", ...) are dropped up
    front. Articles mentioning only OpenAI / ChatGPT / Gemini (no Anthropic), or
    a bare "Claude" as a person's name, are dropped.
    Each tier is ordered newest-first (publishedAt) so a >TOP_N pool yields the
    freshest picks. Returns the full ranked candidate pool, not capped — caller
    picks TOP_N after a separate body-pollution check.
    """
    primary: list[dict] = []
    secondary: list[dict] = []
    ...
    for article in articles:
        ...
        if _is_blacklisted_source(article) or _has_blacklisted_title(article):
            continue
    ...
    # Newest-first within each tier so the TOP_N selection prefers fresh news.
    primary.sort(key=lambda a: a.get("publishedAt") or "", reverse=True)
    secondary.sort(key=lambda a: a.get("publishedAt") or "", reverse=True)
...
def build_messages(items: list[dict]) -> list[str]:
    ...
    header = f"🤖 <b>앤트로픽 뉴스 브리핑</b> ({now_kst})"
...
def main() -> int:
    ...
    candidates = filter_anthropic(raw_articles)
    log.info("=== Anthropic filter passed %d candidate(s) ===", len(candidates))

    if not candidates:
        log.info("No Anthropic articles after filtering — sending 'no news' message.")
        now_kst = datetime.now(KST).strftime("%Y-%m-%d %H:%M KST")
        empty_msg = (
            f"🤖 <b>앤트로픽 뉴스 브리핑</b> ({now_kst})\n\n"
            "오늘 새 앤트로픽 뉴스가 없습니다."
        )
